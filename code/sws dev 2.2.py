    ...
    for url in urls:
        ...
        if dela_byty:
            ...
            for index in range(0, len(velikosti_od)):
                ...
                print(f"čtení počtu bytů - hotovo: {progres}%")
            ...
            progres = 100
            print(f"čtení domů - hotovo: {progres}%")
        dela_byty = False
...
def excel(byty, domy, file_path):
    ...
    # Save the changes to the workbook if the day has not yet been recorded
    if sheet['L2'] != date_cell.value:
        wb.save(file_path)
        print(f"excel taublka uložena zde: {file_path}")
    else:
        print("program byl už dnes spuštěn")
        if test_mode_2 == False:
            quit()
...
def statistiky(iteration):
    ...
    try:
        ...
        print(f"statistika - hotovo: 80%")
        print(f"statistika - hotovo: 90%")
    except:
        pass 
    # Save the changes to the workbook
    wb.save(file_path)
    print(f"statistika - hotovo: 100%")
    #get needed values for email
    print("příprava emailu - hotovo 100%")
